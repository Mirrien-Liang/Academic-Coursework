...
ins <- read.csv("D:\\dropbox\\Dropbox\\functional\\stat452\\stat452\\Insurance.csv", header=TRUE)
...
# Will use 2 reps of 3-fold cross-validation to tune this
#### Tune
...
for (r in 1:R){ 
  for(v in 1:V){
    ...
    for(g in grid3){
      ...
      fit <- randomForest(data=set1.1, per~., ntree=100, 
                              mtry=4, nodesize=g)
...
mod3 <- randomForest(data=set1, per~., ntree=100, 
                              mtry=4, nodesize=grid3[which.min(mean.Mat3)])
